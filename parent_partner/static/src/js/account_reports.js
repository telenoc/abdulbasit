odoo.define('parent_partner.account_report', function (require) {
'use strict';
    ...
    var M2MFilters = Widget.extend(StandaloneFieldManagerMixin, {
        ...
        init: function (parent, fields) {
            ...
            this.fields = fields;
            if(parent && parent.custom_m20){
                this.custom_m20 = parent.custom_m20;
            }else{
                this.custom_m20 = [];
            }
            this.widgets = {};
        ...
        willStart: function () {
            ...
            _.each(this.fields, function (field, fieldName) {
                if(fieldName == 'parent_id_custom_m20'){
                    defs.push(self._makeM2OWidget(field, fieldName,self.custom_m20));
                }else{
        ...
        _confirmChange: function (e) {
            ...
            var comparison_field = ''
            _.each(this.registeredWidgets,function(ev){
                if(ev.parent_id_custom_m20){
                    comparison_field = ev.parent_id_custom_m20.dataPointID
                }
            });
            if(e == comparison_field && this.__parentedParent.report_model == 'account.partner.ledger'){
                this._rpc({
                    model: 'account.report',
                    method: 'get_report_only_parent_ids',
                    args: [,data.parent_id_custom_m20],
                }).then(function(ed){
                    _.each(ed, function (aev) {
                        var res_title = aev.name;
                        var res_id = aev.id;
                        var da_data =  {display_name: res_title,id: res_id}
                        var in_true = true;
                        _.each(data.partner_ids, function (id) {
                            if(id == res_id){
                                in_true = false;
                            }
                        });
                        if(in_true){
                            if(self.widgets && self.widgets.partner_ids){
                                self.widgets.partner_ids._addTag(da_data);
                            }
                        }
                    });
                    _.each(data.partner_ids, function (id) {
                        ...
                        if(is_true){
                            if(self.widgets && self.widgets.partner_ids){
                                self.widgets.partner_ids._removeTag(id);
                            }
                        }
                    });
                });
        ...
        _makeM2OWidget: function (fieldInfo, fieldName,custom_m20) {
            var self = this;
            var options = {};
            options[fieldName] = {
                options: {
            ...
            return this.model.makeRecord(fieldInfo.modelName, [{
                name: 'parent_id_custom_m20',
                type: 'many2one',
                relation: 'res.partner',
                value: fieldInfo.value,
                domain: [['id', 'in', custom_m20]],
            }],options).then(function(recordID){
                self.widgets[fieldName] = new FieldMany2One(self, fieldName, self.model.get(recordID), {
                    mode: 'edit',
                    attrs: {
                        can_create: false,
                        can_write: false,
    ...
    accountReportsWidget.include({
        custom_events: _.extend({}, accountReportsWidget.prototype.custom_events, {
            'value_changed': function(ev) {
                 var self = this;
                 if(this.report_model == 'account.partner.ledger'){
                    self.report_options.parent_id_custom_m20 = ev.data.parent_id_custom_m20;
                 }
        ...
        render_searchview_buttons: function() {
            ...
            if (this.report_options.partner) {
                if (!this.M2MFilters) {
                    var fields = {};
                    if ('parent_id_custom_m20' in this.report_options && this.report_model == 'account.partner.ledger') {
                        fields['parent_id_custom_m20'] = {
                            label: _t('Parent'),
                            modelName: 'res.partner',
                            value: this.report_options.parent_id_custom_m20,
                        };
                    }
                    ...
                    if ('partner_categories' in this.report_options) {
                        ...
                        if (!_.isEmpty(fields)) {
                            self._rpc({
                                model: 'res.partner',
                                method: 'get_parent_id_custom_m20_val',
                                args: [0],
                            }).then(function(custom_m20){
                                self.custom_m20 = custom_m20;
                                self.M2MFilters = new M2MFilters(self, fields);
                                self.M2MFilters.appendTo(self.$searchview_buttons.find('.js_account_partner_m2m'));
                            });
                        }
                    }
                } else {
